...
- **Any value a user can write is untrusted at every READ site, not just at the write.** A validated write path does not sanctify the column: table-level grants mean users can write columns directly via PostgREST, bypassing every server action. At render/use time, PARSE the stored value and REBUILD from trusted parts (the row's own id, a digits-only capture) — never concatenate a stored string into a URL, path, or identifier. On record (2026-08-26, trainer-images arc): profiles.avatar_url concatenated into an image URL let any user render a victim's photo as their own on the public directory; the fix is publicAvatarUrl's parse-and-rebuild (lib/images/avatar.ts)
- **NEVER report an outcome from a proxy** — a script's exit message, a summary, a filtered view, or your own narration of what you were about to do. Verify against the artifact itself (the file on disk, the DOM, the catalog, the deployed page) before claiming something landed. Three instances of this failure shape are on record: merges narrated before merging; a `RESEND|EMAIL|SUPABASE`-filtered env grep reported as a full read that "proved" absent Turnstile keys (2026-07-31); a batch edit script trusted from its own "ok" log while three of its edits never landed because it aborted mid-run (2026-08-14). The fourth instance (2026-08-14: "all pages swapped" verified by grepping PAGES missed the 27th instance living in a component) adds the scope facet: **match the verification's scope to the claim's scope** — if you claim "all X," search everywhere X can live, not just where you expect it. The special case — no "there is no X" claims from filtered output — still applies: absence claims require the FULL listing
